...
const platforms = [
  ...
  },
  {
    name: "Facebook",
    color: "#1877F2",
    gradient: "linear-gradient(180deg, #1877F2 0%, #0C5DC7 100%)",
    icon: (
      <svg viewBox="0 0 24 24" className="h-6 w-6 md:h-8 md:w-8" fill="currentColor">
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6">
      {platforms.map((platform, index) => (
        ...
        >
          <div
            className={`${sizeClasses[size]} rounded-2xl flex items-center justify-center shadow-lg transition-all`}
            style={{ 
              background: platform.gradient,
              color: platform.name === "TikTok" ? "white" : "white"
            }}
          >
            {platform.icon}
          </div>
